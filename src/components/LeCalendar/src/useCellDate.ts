...
export const useCellDate = (
  ...
  const isToday = computed(
    () => formatDate(props.cellDate!, formatDateType) === formatDate(props.today!, formatDateType)
  )
  ...
  const isSelected = computed(() => formatDate(props.cellDate!, formatDateType) === valueStr.value)
  const type = computed(() =>
    isCurrent.value ? 'current-month' : isPrev.value ? 'prev-month' : 'next-month'
  )
