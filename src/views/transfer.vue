  ...
  <dl>
    ...
    <dd>
      <LeTransfer
        ...
        @change="handleChange"
      >
        <template #default="{ option }"> {{ option.key }}-{{ option.label }} </template>
...
import type { TransferDataItem, TransferKey } from '@/components/LeTransfer/src/typing'
...
const value = ref([data.value[0].key, data.value[1].key, data.value[2].key, data.value[3].key])
...
function handleChange(currentKeys: TransferKey[], dir: Direction, targetKeys: TransferKey[]) {
